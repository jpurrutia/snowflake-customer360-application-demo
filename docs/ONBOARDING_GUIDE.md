...
- **Silver**: Cleaned, standardized data (quality checks)
- **Gold**: Business-ready aggregates (star schema, marts)

**Star Schema**
- **Fact Table**: Transaction-level data (fct_transactions: 13.5M rows)
- **Dimension Tables**: Descriptive attributes (dim_customer, dim_date, dim_merchant_category)
- **Benefits**: Optimized for analytical queries, intuitive for business users

**SCD Type 2** (Slowly Changing Dimension)
- Tracks historical changes with valid_from/valid_to dates
- Example: Customer upgrades from Standard to Premium card
- Enables time-travel queries (what did customer look like on 2024-01-15?)

### Customer Segmentation

**5 Behavioral Segments**:
1. **High-Value Travelers** (10-15%): Premium cards, $5K+/month, 25%+ travel
...
- Dynamic 90-day calculations that update daily
- Examples: spend_last_90_days, transactions_last_90_days
- Enables trend analysis (is customer increasing or decreasing spend?)

### Machine Learning

**Churn Definition**
- Customer has churned if **either**:
...
- **Precision**: Of predicted churners, % actually churned (≥0.60)
- **Recall**: Of actual churners, % we identified (≥0.40)

**Churn Risk Categories**
- **Low Risk** (0-39): 70-80% of customers
- **Medium Risk** (40-69): 15-25% of customers
- **High Risk** (70-100): 5-10% of customers

### Snowflake Features

**Cortex ML**
- Managed ML service (no infrastructure management)
...
- Maps business terms to SQL concepts
- Returns both SQL and results

**Streamlit in Snowflake**
- Hosted Streamlit within Snowflake
...
SELECT churn_risk_category, COUNT(*) as customer_count
FROM GOLD.CUSTOMER_360_PROFILE
GROUP BY churn_risk_category;

-- Top 10 customers by LTV
SELECT customer_id, full_name, lifetime_value, customer_segment
FROM GOLD.CUSTOMER_360_PROFILE
...
SELECT
    ...
    COUNT(lifetime_value) as ltv_count,
    COUNT(churn_risk_score) as churn_score_count
FROM GOLD.CUSTOMER_360_PROFILE;
...
| Task | Command |
|------|---------|
| **Start Streamlit** | `cd streamlit && streamlit run app.py` |
| **Run dbt** | `cd dbt_customer_analytics && dbt run` |
| **Test dbt** | `cd dbt_customer_analytics && dbt test` |
| **Run integration tests** | `pytest tests/integration/ -v` |
| **Connect to Snowflake** | `snowsql -a <account> -u <user>` |
| **Generate docs** | `cd dbt_customer_analytics && dbt docs generate && dbt docs serve` |
| **Check query history** | Log into Snowsight → Activity → Query History |
| **Clear Streamlit cache** | `streamlit cache clear` |

**Need help?** Check the [Troubleshooting](#troubleshooting) section or review iteration summaries in `docs/prompt_5.*_completion_summary.md`.
